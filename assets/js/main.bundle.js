(() => {
    ...
    ["_initWebGL"]() {
      try {
        ...
        if (!gl) gl = this.canvas.getContext('webgl') || this.canvas.getContext('experimental-webgl');
        if (!gl) { this.useWebGL = false; return; }
        this.gl = gl;
        const vsSrc = "attribute vec2 aPos; attribute vec2 aUV; varying vec2 vUV; void main(){ vUV=aUV; gl_Position=vec4(aPos,0.0,1.0); }";
        const fsSrc = "precision mediump float; varying vec2 vUV; uniform sampler2D uSampler; void main(){ gl_FragColor = texture2D(uSampler, vUV); }";
        const prog = this._createGLProgram(gl, vsSrc, fsSrc);
        gl.useProgram(prog);
        ...
        this._gl = { program: prog, vbo, tex, aPos, aUV, uSampler };
        gl.clearColor(0,0,0,1);
        gl.enable(gl.BLEND);
        gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
      } catch (e) {
        console.warn('Failed to init WebGL, fallback to 2D', e);
        this.useWebGL = false; this.gl = null;
      }
    }
    ["_createGLProgram"](gl, vsSrc, fsSrc) {
      function compile(t, s){ const sh=gl.createShader(t); gl.shaderSource(sh, s); gl.compileShader(sh); if(!gl.getShaderParameter(sh, gl.COMPILE_STATUS)){ const log=gl.getShaderInfoLog(sh); gl.deleteShader(sh); throw new Error(log||'shader error'); } return sh; }
      const vs = compile(gl.VERTEX_SHADER, vsSrc);
      const fs = compile(gl.FRAGMENT_SHADER, fsSrc);
      const prog = gl.createProgram();
      gl.attachShader(prog, vs); gl.attachShader(prog, fs); gl.linkProgram(prog);
      if(!gl.getProgramParameter(prog, gl_LINK_STATUS = gl.LINK_STATUS)) { const log = gl.getProgramInfoLog(prog); gl.deleteProgram(prog); throw new Error(log||'link error'); }
    ...
    ["_glBlit"](srcCanvas) {
      ...
      gl.bindTexture(gl.TEXTURE_2D, this._gl.tex);
      gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, srcCanvas);
      gl.clear(gl.COLOR_BUFFER_BIT);
      gl.useProgram(this._gl.program);
      gl.bindBuffer(gl.ARRAY_BUFFER, this._gl.vbo);
      gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
    }

    ["_ensureGlWorld"]() {
      if (this._glWorld) return this._glWorld;
      const gl = this.gl; if (!gl) return null;
      const vs = `
        attribute vec2 aPos; // world position
        attribute float aSize; // world radius
        attribute vec3 aColor;
        varying vec3 vColor;
        uniform vec2 uCam; // camera center
        uniform float uView; // viewScale
        uniform vec2 uCanvas; // canvas size in pixels
        void main(){
          float sx = ((aPos.x - uCam.x) / uView) + (uCanvas.x * 0.5);
          float sy = ((aPos.y - uCam.y) / uView) + (uCanvas.y * 0.5);
          float clipX = (sx / uCanvas.x) * 2.0 - 1.0;
          float clipY = (sy / uCanvas.y) * 2.0 - 1.0;
          gl_Position = vec4(clipX, clipY, 0.0, 1.0);
          gl_PointSize = max(1.0, (2.0 * aSize) / uView);
          vColor = aColor;
        }
      `;
      const fs = `
      ...
      `;
      const prog = this._createGLProgram(gl, vs, fs);
      const aPos = gl.getAttribLocation(prog, 'aPos');
      ...
      const vbo = gl.createBuffer();
      this._glWorld = { prog, aPos, aSize, aColor, uCam, uView, uCanvas, vbo };
      return this._glWorld;
    }

    ["_glRenderWorld"](pellets, cells) {
      const gl = this.gl; if (!gl) return;
      const gw = this._ensureGlWorld(); if (!gw) return;
      gl.viewport(0, 0, this.canvas.width|0, this.canvas.height|0);
      gl.useProgram(gw.prog);
      gl.uniform2f(gw.uCam, this.camera.x, this.camera.y);
      gl.uniform1f(gw.uView, this.camera.viewScale);
      gl.uniform2f(gw.uCanvas, this.canvas.width, this.canvas.height);

      const total = (pellets.length + cells.length);
      const stride = 6;
      const data = new Float32Array(total * stride);
      let idx = 0;
      const pushCell = (c) => {
        const r = c.colorRGB ? c.colorRGB[0] : 1.0;
        const g = c.colorRGB ? c.colorRGB[1] : 1.0;
        const b = c.colorRGB ? c.colorRGB[2] : 1.0;
        data[idx++] = c.x;
        data[idx++] = c.y;
        data[idx++] = c.size;
        data[idx++] = r;
      ...
      for (let i=0;i<pellets.length;i++) pushCell(pellets[i]);
      for (let i=0;i<cells.length;i++) pushCell(cells[i]);

      gl.bindBuffer(gl.ARRAY_BUFFER, gw.vbo);
      gl.bufferData(gl.ARRAY_BUFFER, data, gl.DYNAMIC_DRAW);
      const BYTES = 4;
      const strideBytes = stride * BYTES;
      gl.enableVertexAttribArray(gw.aPos);
      gl.vertexAttribPointer(gw.aPos, 2, gl.FLOAT, false, strideBytes, 0);
      gl.enableVertexAttribArray(gw.aSize);
      gl.vertexAttribPointer(gw.aSize, 1, gl.FLOAT, false, strideBytes, 2*BYTES);
      gl.enableVertexAttribArray(gw.aColor);
      gl.vertexAttribPointer(gw.aColor, 3, gl.FLOAT, false, strideBytes, 3*BYTES);
    ...
    ["_glRenderWorld"](pellets, cells) {
      ...
      let idx = 0;
      const pushCell = (c) => {
        // approximate color; if RGB array present, use it; else fallback to white
        const r = c.colorRGB ? c.colorRGB[0] : 1.0;
        const g = c.colorRGB ? c.colorRGB[1] : 1.0;
        const b = c.colorRGB ? c.colorRGB[2] : 1.0;
        data[idx++] = c.x;
    ...
    ["_ensureGlWorld"]() {
      if (this._glWorld) return this._glWorld;
      const gl = this.gl; if (!gl) return null;
      // A simple shader that draws circles using gl.POINTS with per-vertex size and color, transforming world->clip
      const vs = `
        attribute vec2 aPos; // world position
        attribute float aSize; // world radius
        attribute vec3 aColor;
        ...
        uniform vec2 uCanvas; // canvas size in pixels
        void main(){
          // world to NDC: ((world - cam)/view) to pixels then to clip
          vec2 screen = ((aPos - uCam) / uView);
          // convert to pixels
          screen = screen + vec2(uCanvas.x*0.5, uCanvas.y*0.5) / uView; // not exact; we convert below using canvas
          // Proper transform: world->screen pixels
          float sx = ((aPos.x - uCam.x) / uView) + (uCanvas.x * 0.5);
          float sy = ((aPos.y - uCam.y) / uView) + (uCanvas.y * 0.5);
          // to clip
          float clipX = (sx / uCanvas.x) * 2.0 - 1.0;
          float clipY = (sy / uCanvas.y) * 2.0 - 1.0;
          gl_Position = vec4(clipX, clipY, 0.0, 1.0);
          // point size in pixels: diameter = 2*radius / viewScale
          gl_PointSize = max(1.0, (2.0 * aSize) / uView);
          vColor = aColor;
        }
      `;
    ...
    ["_glRenderWorld"](pellets, cells) {
      ...
      // Build interleaved buffer: [pos.x, pos.y, size, r, g, b]
      const total = (pellets.length + cells.length);
      const stride = 6; // floats
      const data = new Float32Array(total * stride);
      let idx = 0;
      const pushCell = (c) => {
        const r = c.colorRGB ? c.colorRGB[0] : (c.color ? 1.0 : 0.2);
        const g = c.colorRGB ? c.colorRGB[1] : (c.color ? 1.0 : 0.2);
        const b = c.colorRGB ? c.colorRGB[2] : (c.color ? 1.0 : 0.2);
        data[idx++] = c.x; // world position (current draw uses x/y; for animation, targetX/Y could be used)
        data[idx++] = c.y;
        data[idx++] = c.size;
        data[idx++] = r;
        data[idx++] = g;
    ...
    const _0x518936 = new class {
      constructor() {
        ...
        this.useWebGL = false;
        this.gl = null;
        this._gl = { program: null, vbo: null, tex: null, aPos: -1, aUV: -1, uSampler: null };
        this._glWorld = null;
      ...
      ["start"]() {
        ...
        // Force WebGL to be disabled - always use Canvas 2D
        this.useWebGL = false;
        
        try {
          // Check settings but ignore any WebGL enabling attempts
          if (window.app && window.app.settings && window.app.settings.useGpuRenderer) {
            console.warn('WebGL requested but disabled by default - using Canvas 2D');
          }
          if (!this.useWebGL) {
            try {
              ...
              if (raw) {
                ...
                if (s.useGpuRenderer) {
                  console.warn('WebGL setting found but disabled by default - using Canvas 2D');
                }
                // Force override any WebGL setting
                this.useWebGL = false;
              }
            } catch (_) {}
          }
      ...
      ["setZoom"](_0x3c8a35) {
        // Wheel down (deltaY > 0) => zoom out (decrease zoom)
        // Wheel up (deltaY < 0) => zoom in (increase zoom), capped at 2.0
        const dy = (typeof _0x3c8a35.deltaY === 'number') ? _0x3c8a35.deltaY : (0 - (_0x3c8a35.wheelDelta || 0));
        const steps = -dy / 120; // Inverted: dy>0 => zoom out, dy<0 => zoom in
        let nextZoom = this.camera.zoom * Math.pow(1.1, steps);
        // Cap only maximum zoom-in to avoid extreme magnification; no hard cap on zoom-out
        this.camera.zoom = Math.min(nextZoom, 2.0);
      }
      ['updateZoom']() {
      ...
      // Draw background image inside arena bounds (overlay)
      ...
      try {
        ...
        let border = { left: -0x1f40, top: -0x1f40, right: 0x1f40, bottom: 0x1f40 };
        if (active && active.border) border = active.border;

        const bw = border.right - border.left;
        const bh = border.bottom - border.top;
        if (bw <= 0 || bh <= 0) return;

        const arenaRatio = bw / bh;
        const imgRatio = iw / ih;

        let sx = 0, sy = 0, sWidth = iw, sHeight = ih;
        if (imgRatio > arenaRatio) {
          const newW = ih * arenaRatio;
          sx = Math.max(0, (iw - newW) / 2);
          sWidth = newW;
        } else {
          const newH = iw / arenaRatio;
          sy = Math.max(0, (ih - newH) / 2);
          sHeight = newH;
        }
        ...
        this.ctx.drawImage(img, sx, sy, sWidth, sHeight, border.left, border.top, bw, bh);
        this.ctx.restore();
      } catch (e) {
        console.warn("Failed to draw overlay background:", e);
      }
}
      ['drawGrid']() {
        this.ctx.save();
        this.ctx.lineWidth = 0x1;
        this.ctx.strokeStyle = "#404040"; // Grey grid color (changed from bluish)
      ...
      ["drawSectors"]() {
        ...
        this.ctx.strokeStyle = "#404040"; // Grey color for sectors (changed from blue)
        ...
        // Use grey/black color for sector labels (no glow)
        this.ctx.fillStyle = "#303030"; // Dark grey color (changed from bluish gradient)
        this.ctx.font = "700 740px ubuntu";
        this.ctx.textAlign = "center";
        this.ctx.textBaseline = "middle";
        
        // No glow effect for sector labels as requested
      ...
      ["drawMapBorder"]() {
        ...
        // Save context state to prevent flickering
        this.ctx.save();
        
        // Create grey border effect (no glow)
        this.ctx.lineWidth = 0x32;
        this.ctx.lineCap = 'round';
        this.ctx.lineJoin = "round";
        
        // Remove glow effect and use grey color
        this.ctx.shadowColor = 'transparent';
        this.ctx.shadowBlur = 0;
        this.ctx.strokeStyle = '#505050'; // Grey border (changed from bright blue)
        this.ctx.globalAlpha = 1.0; // Ensure full opacity to prevent flickering
      ...
      ["drawMinimap"]() {
        ...
        _0x3c3521.strokeStyle = "rgba(255, 255, 255, 0.2)";
        _0x3c3521.fillStyle = "rgba(255, 255, 255, 0.6)";
        _0x3c3521.textBaseline = "middle";
        _0x3c3521.textAlign = "center";
        
        // Draw grid with proper arena coordinates (A-E, 1-5) - completely new horizontal system
        _0x3c3521.save();
        
        const fontSize = Math.min(_0x125327, _0x3d9177) / 3.5;
        _0x3c3521.font = "600 " + fontSize + "px 'Segoe UI', Arial, sans-serif";
        _0x3c3521.fillStyle = "rgba(255, 255, 255, 0.6)";
        _0x3c3521.textAlign = "center";
        _0x3c3521.textBaseline = "middle";
        
        // Create horizontal text for each grid cell
        const gridLabels = [
          ['A1', 'A2', 'A3', 'A4', 'A5'],
          ['B1', 'B2', 'B3', 'B4', 'B5'],
          ['C1', 'C2', 'C3', 'C4', 'C5'],
        ...
        for (let row = 0; row < 5; row++) {
          for (let col = 0; col < 5; col++) {
            const x = (col + 0.5) * _0x125327;
            const y = (row + 0.5) * _0x3d9177;
            
            // Draw text horizontally
            _0x3c3521.fillText(gridLabels[row][col], x, y);
          }
        }
        ...
        _0x3c3521.fillStyle = "rgba(255, 255, 255, 0.15)";
        _0x3c3521.fillRect(_0x262129 * _0x125327, _0x5c9d45 * _0x3d9177, _0x125327, _0x3d9177);
        
        // Draw current position indicator
        _0x3c3521.fillStyle = "#FFD700";
        _0x3c3521.beginPath();
        _0x3c3521.arc(_0x1f52f3, _0x4bf864, 0x6, 0x0, 0x2 * Math.PI);
        _0x3c3521.fill();
        _0x3c3521.strokeStyle = "#FFA500";
        _0x3c3521.lineWidth = 2;
        _0x3c3521.stroke();
        ...
        _0x510813.forEach(_0x339e72 => {
          ...
          // Check if this is parent or child client
          const isParentClient = _0x339e72.clientID === _0xa630e8.getParent()?.clientID;
          const isChildClient = _0x339e72.clientID === _0xa630e8.getChild()?.clientID;
          const isMyClient = _0x339e72.clientID === _0x4dd6bc?.clientID;
          
          if (isParentClient || isChildClient || isMyClient) {
            // Special styling for parent/child cells
            _0x3c3521.fillStyle = isParentClient ? "#00FF88" : (isChildClient ? "#FF6BFF" : "#FFD700");
            _0x3c3521.beginPath();
            _0x3c3521.arc(_0x5f5132, _0x35a041, 0x8, 0x0, 0x2 * Math.PI);
            _0x3c3521.fill();
            
            // Add glow effect
            _0x3c3521.shadowColor = isParentClient ? "#00FF88" : (isChildClient ? "#FF6BFF" : "#FFD700");
            _0x3c3521.shadowBlur = 10;
            _0x3c3521.strokeStyle = isParentClient ? "#00CC66" : (isChildClient ? "#CC44CC" : "#FFA500");
            _0x3c3521.lineWidth = 2;
            _0x3c3521.stroke();
            _0x3c3521.shadowBlur = 0;
            
            // Add special label
            _0x3c3521.font = "bold 10px 'Segoe UI', Arial, sans-serif";
            _0x3c3521.textAlign = "center";
            _0x3c3521.textBaseline = "bottom";
            _0x3c3521.strokeStyle = '#000';
            _0x3c3521.lineWidth = 2;
            const label = isParentClient ? "PARENT" : (isChildClient ? "CHILD" : "YOU");
            _0x3c3521.strokeText(label, _0x5f5132, _0x35a041 - 0xa);
            _0x3c3521.fillStyle = isParentClient ? "#00FF88" : (isChildClient ? "#FF6BFF" : "#FFD700");
            _0x3c3521.fillText(label, _0x5f5132, _0x35a041 - 0xa);
          } else {
            // Regular cells
            ...
            _0x3c3521.font = "11px 'Segoe UI', Arial, sans-serif";
            _0x3c3521.textAlign = "center";
            _0x3c3521.textBaseline = "bottom";
            _0x3c3521.strokeStyle = '#000';
            _0x3c3521.lineWidth = 2;
            ...
            // Use color names if enabled
            if (_0x2395ab.settings.colorNames) {
              _0x3c3521.fillStyle = _0x2395ab.getNicknameColor(_0x339e72.nickname);
            } else {
      ...
      ['drawLeaderboard']() {
        ...
        _0x16162f.font = "900 " + 0x1d * _0x3db33f + "px 'Georgia', serif";
        _0x16162f.letterSpacing = "2px";
        const _0x11bcd3 = _0x16162f.measureText("ProtonX").width;
        _0x16162f.fillText("ProtonX", (_0x18b203.width - _0x11bcd3) / 0x2, 0x20 * _0x3db33f);
        ...
        _0x227fca.items.slice(0x0, 0xa).forEach((_0xc70d43, _0xb54417) => {
          ...
          const posText = " " + _0xc70d43.pos + ".  ";
          const massText = "[" + (_0x3a0926 >= 0x3e8 ? (_0x3a0926 / 0x3e8).toFixed(0x1) + 'k' : _0x3a0926.toString()) + "]  ";
          _0x16162f.font = "700 " + 0x10 * _0x3db33f + "px Ubuntu";
          _0x16162f.fillStyle = "#FFFFFF";
          _0x16162f.fillText(posText, 0xa * _0x3db33f, _0x550480);
          _0x16162f.fillStyle = '#FFD700';
          _0x16162f.fillText(massText, 0xa * _0x3db33f + _0x16162f.measureText(posText).width, _0x550480);
          _0x16162f.fillStyle = _0xc70d43.isMe ? '#FFD700' : "#FFFFFF";
          _0x16162f.fillText(_0xc70d43.name, 0xa * _0x3db33f + _0x16162f.measureText(posText + massText).width, _0x550480);
        });
      }
...
["updateScene"](_0xc209c9) {
  ...
  this.ctx = bufferCtx;
  const ctx = bufferCtx;
  const doWebGLWorld = this.useWebGL && this.gl;
  ...
  // Prepare frames
    ...
    // Camera & draw
    if (!(doWebGLWorld && typeof this._glRenderWorld === 'function')) {
      ctx.save();
      this.setCamera();
      // Draw background overlay within arena bounds first so grids/sectors draw above it
      try {
        this.drawBackgroundOverlay();
      } catch (e) {
        console.warn('Overlay background draw failed during buffer rendering', e);
      }
      ...
      try {
        // Clear once for GL frame, then draw world then overlays (grid/border, cursor lines)
        if (this.gl) {
          this.gl.clearColor(0,0,0,1);
          this.gl.clear(this.gl.COLOR_BUFFER_BIT);
        }
        this._glRenderWorld(this.pelletsFrame, this.cellsFrame);
        this._glDrawSectorsAndBorder();
        if (_showCursorLines) this._glDrawCursorLines((_0x2395ab.settings && _0x2395ab.settings.cursorLinesColor) || '#FFFFFF');
      } catch (e) {
        console.warn('GL world render failed, falling back to 2D this frame', e);
        ctx.save();
        this.setCamera();
        if (_showSectors) this.drawSectors();
        this.drawMapBorder();
    ...
    if (_showCursorLines && !doWebGLWorld) {
      try {
        const active = _0xa630e8.getActiveClient();
    ...
    // Restore and present to visible canvas
    this.ctx = mainCtx;
    if (!doWebGLWorld) {
...
["loop"]() {
    ...
    ["_initWebGL"]() {
      try {
        ...
        this._gl = { program: prog, vbo, tex, aPos, aUV, uSampler };
        gl.clearColor(0,0,0,1);
        // Enable alpha blending for world rendering overlays
        gl.enable(gl.BLEND);
        gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
      } catch (e) {
        console.warn('Failed to init WebGL, fallback to 2D', e);
    ...
    ["_createGLProgram"](gl, vsSrc, fsSrc) {
      ...
      const prog = gl.createProgram();
      gl.attachShader(prog, vs); gl.attachShader(prog, fs); gl.linkProgram(prog);
      if(!gl.getProgramParameter(prog, gl.LINK_STATUS)) { const log = gl.getProgramInfoLog(prog); gl.deleteProgram(prog); throw new Error(log||'link error'); }
      gl.deleteShader(vs); gl.deleteShader(fs);
      return prog;
    }
    ["_glBlit"](srcCanvas) {
    ...
    ["_ensureGlLines"]() {
      if (this._glLines) return this._glLines;
      const gl = this.gl; if (!gl) return null;
      const vs = `
        attribute vec2 aPos;
        uniform vec2 uCam; uniform float uView; uniform vec2 uCanvas;
        void main(){
          float sx = ((aPos.x - uCam.x) / uView) + (uCanvas.x * 0.5);
          float sy = ((aPos.y - uCam.y) / uView) + (uCanvas.y * 0.5);
          float clipX = (sx / uCanvas.x) * 2.0 - 1.0;
      ...
      const vbo = gl.createBuffer();
      this._glLines = { prog, aPos, uCam, uView, uCanvas, uColor, vbo };
      return this._glLines;
    }
    ["_glDrawLines"](pointsArray, color) {
      const gl = this.gl; if (!gl) return;
      const ls = this._ensureGlLines(); if (!ls) return;
      gl.useProgram(ls.prog);
      gl.uniform2f(ls.uCam, this.camera.x, this.camera.y);
      gl.uniform1f(ls.uView, this.camera.viewScale);
      gl.uniform2f(ls.uCanvas, this.canvas.width, this.canvas.height);
      gl.uniform4f(ls.uColor, color[0], color[1], color[2], color[3]);
      const data = new Float32Array(pointsArray);
      gl.bindBuffer(gl.ARRAY_BUFFER, ls.vbo);
      gl.bufferData(gl.ARRAY_BUFFER, data, gl.DYNAMIC_DRAW);
      gl.enableVertexAttribArray(ls.aPos);
      gl.vertexAttribPointer(ls.aPos, 2, gl.FLOAT, false, 8, 0);
      gl.drawArrays(gl.LINES, 0, data.length / 2);
    }
    ["_glDrawSectorsAndBorder"]() {
      const active = _0xa630e8.getActiveClient();
      let border = { left: -0x1f40, top: -0x1f40, right: 0x1f40, bottom: 0x1f40 };
      if (active) border = active.border;
      const lines = [];
      const show = !!(_0x2395ab.settings && _0x2395ab.settings.showSectors);
      if (show) {
        const cols = 5, rows = 5;
        const stepX = (border.right - border.left) / cols;
        const stepY = (border.bottom - border.top) / rows;
        for (let i=0;i<=cols;i++) {
          const x = border.left + i*stepX;
          lines.push(x, border.top, x, border.bottom);
        }
        for (let j=0;j<=rows;j++) {
          const y = border.top + j*stepY;
          lines.push(border.left, y, border.right, y);
        }
      }
      if (lines.length) this._glDrawLines(lines, [0.25,0.25,0.25,1.0]); // Grey sectors color (changed from bluish)
      
      // Draw border with consistent opacity to prevent flickering
      const m = 0x32;
      const bx0 = border.left - m, by0 = border.top - m, bx1 = border.right + m, by1 = border.bottom + m;
      const b = [
        bx0, by1, bx1, by1,
        bx1, by1, bx1, by0,
        bx1, by0, bx0, by0,
      ...
      // Use grey border color (changed from bright blue)
      this._glDrawLines(b, [0.31,0.31,0.31,1.0]);
      
      // Note: "ProtonX" text is rendered in the 2D fallback path in drawMapBorder function
      // WebGL text rendering would require additional texture/font setup
    }
    ["_glDrawCursorLines"](colorHex) {
      try {
        const active = _0xa630e8.getActiveClient();
        if (!active || !active.stores || !active.stores.ownedCells || !active.stores.ownedCells.length) return;
        const lines = [];
        for (let i=0;i<active.stores.ownedCells.length; i++) {
          const c = active.stores.ownedCells[i]; if (!c) continue;
          lines.push(c.x, c.y, this.mouse.worldX, this.mouse.worldY);
        }
        const col = (function parse(hex){
          if (!hex || typeof hex !== 'string') return [1,1,1,0.9];
          let h = hex.replace('#','');
    ...
    const _0x337cc2 = new class {
      ...
      ["drawNickname"](_0x2de972, _0x3b6903, _0x92cf7a, _0x80e999) {
        ...
        // Use color names if enabled
        if (_0x2395ab.settings.colorNames) {
          _0x6275a.fillStyle = _0x2395ab.getNicknameColor(_0x2de972.nickname);
        } else {
      ...
      ["drawMass"](_0x151c6c, _0x281df9, _0x2cc7ca, _0x1faa62) {
        ...
        if (_0x263292.needsRedraw) {
          ...
          // Use same color as nickname when colorNames is enabled
          if (_0x2395ab.settings.colorNames && _0x151c6c.nickname) {
            _0x1566ba.fillStyle = _0x2395ab.getNicknameColor(_0x151c6c.nickname);
          } else {
    ...
    class _0x270f21 {
      ...
      ["drawCell"](_0x3c496a) {
        ...
        // Check multiple ways to ensure 3D effect works
        let is3DEnabled = false;
        
        // Check settings object
        if (_0x2395ab.settings && _0x2395ab.settings.enable3DEffect) {
          is3DEnabled = true;
        }
        // Check localStorage as fallback
        else {
          try {
            const savedState = localStorage.getItem('enable3DEffect');
            if (savedState === 'true') {
              is3DEnabled = true;
            }
          } catch (e) {}
        ...
        if (is3DEnabled && !this.flags.isPellet) {
          // Save context state
          _0x3c496a.save();
          
          // Optimized 3D effect - solid shadow
          _0x3c496a.shadowColor = 'rgba(0, 0, 0, 0.8)';
          _0x3c496a.shadowBlur = Math.min(this.size * 0.1, 8); // Cap shadow blur for performance
          _0x3c496a.shadowOffsetX = Math.min(this.size * 0.05, 4); // Cap offset for performance
          _0x3c496a.shadowOffsetY = Math.min(this.size * 0.05, 4); // Cap offset for performance
          
          // Get base color
          const fillColor = (cellColorSelect === 'default') ? this.color : cellColorSelect;
          
          // Simplified gradient for better performance
          const gradient = _0x3c496a.createRadialGradient(
          ...
          // Simplified gradient stops - darkening effect only
          gradient.addColorStop(0, this.darkenColor(fillColor, 10));
          gradient.addColorStop(0.5, this.darkenColor(fillColor, 20));
          gradient.addColorStop(1, this.darkenColor(fillColor, 35));
          
          _0x3c496a.fillStyle = gradient;
          _0x3c496a.beginPath();
        ...
        if (!(this.flags.isPellet && this.flags.isEject && this.flags.isVirus)) {
          ...
          if (_0x578b35 && _0x29160e > 0x1 && !this.flags.isPellet && !this.flags.isEject && !this.flags.isVirus) {
            ...
            // Check if arrow rings are enabled
            if (_0x2395ab.settings && _0x2395ab.settings.arrowRings) {
                // Arrow rings mode: show arrow only for selected cell, no rings
                if (_0x433719) {
                    // Draw arrow for selected cell
                    _0x3c496a.save();
                    
                    // Calculate arrow size based on cell size and user setting
                    const scaleFactor = arrowSize / 200; // Normalize to default 200px
                    const scaledArrowSize = Math.max(this.size * 0.3 * scaleFactor, 20 * scaleFactor); // Scale with cell and user setting
                    _0x3c496a.translate(this.x, this.y - this.size - scaledArrowSize);  // Distance based on scaled size
                    
                    // Add subtle glow effect
                    _0x3c496a.shadowColor = arrowColor;
                    _0x3c496a.shadowBlur = Math.min(scaledArrowSize/20, 5);  // Reduced glow for performance
                    _0x3c496a.fillStyle = arrowColor;
                    
                    // Draw smaller arrow head (triangle) - scaled with cell
                    _0x3c496a.beginPath();
                    _0x3c496a.moveTo(0, scaledArrowSize/3);  // Smaller height
                    _0x3c496a.lineTo(-scaledArrowSize/3, -scaledArrowSize/3);  // Smaller width
                    _0x3c496a.lineTo(scaledArrowSize/3, -scaledArrowSize/3);   // Smaller width
                    _0x3c496a.closePath();
                    _0x3c496a.fill();
                    _0x3c496a.restore();
                }
                // Don't draw any rings when arrow rings is enabled
            } else {
                // Original multibox rings mode
      ...
      ["drawVirus"](_0x5cb68d) {
        ...
        // Bucket size to limit number of cached canvases and reuse similar sizes.
        const rawSize = this.size * _0x584fcf.width / 0x64;
        const bucket = Math.max(8, Math.round(rawSize / 8) * 8);
        const _0x21653d = bucket;
        const _0x_x = this.x - _0x21653d / 0x2;
        const _0x_y = this.y - _0x21653d / 0x2;

        // Global cache on window to persist across frames.
        if (!window.__virusTintCache) window.__virusTintCache = Object.create(null);
        const cache = window.__virusTintCache;

        // If cached pre-render exists, draw it directly (fast)
        if (cache[bucket]) {
          _0x5cb68d.drawImage(cache[bucket], _0x_x, _0x_y, _0x21653d, _0x21653d);
          return;
        }

        // Create offscreen canvas, draw and tint once, then cache.
        try {
          const off = document.createElement('canvas');
          const sizeInt = Math.max(8, Math.round(_0x21653d));
          off.width = sizeInt;
          off.height = sizeInt;
          const ctx = off.getContext('2d');

          // Draw base virus scaled to canvas
          ctx.drawImage(_0x584fcf, 0, 0, sizeInt, sizeInt);

          // Glow: lighter composite with translucent red circle
          ctx.save();
          ctx.globalCompositeOperation = 'lighter';
          ctx.fillStyle = 'rgba(255,0,0,0.12)';
          const glowRadius = Math.max(2, Math.min(8, this.size * 0.03));
          // Draw slightly larger circle for glow
          ctx.beginPath();
          ctx.arc(sizeInt/2, sizeInt/2, sizeInt/2 * 0.98, 0, Math.PI * 2);
          ctx.fill();
          ctx.restore();

          // Stronger tint only over image area using source-atop
          ctx.save();
          ...
          // Cache and draw
          cache[bucket] = off;
          _0x5cb68d.drawImage(off, _0x_x, _0x_y, _0x21653d, _0x21653d);
        } catch (e) {
          // Fallback: draw directly if offscreen creation fails
          _0x5cb68d.drawImage(_0x584fcf, _0x_x, _0x_y, rawSize, rawSize);
        }
      }
      ["drawSkin"](_0x3e8565) {
    ...
    const _0x2395ab = new class {
      constructor() {
        ...
        // Color generation for nicknames
        this.nicknameColors = new Map();
        this.generateNicknameColors();
      ...
      ["generateNicknameColors"]() {
        this.nicknameColors.clear();
        const darkColors = [
          '#8B4513', '#A0522D', '#D2691E', '#CD853F', '#DEB887',
          '#F4A460', '#DAA520', '#B8860B', '#FF8C00', '#FF6347',
          '#DC143C', '#B22222', '#8B0000', '#800000', '#483D8B',
          '#4B0082', '#6A5ACD', '#7B68EE', '#4682B4', '#191970',
          '#006400', '#2E8B57', '#3CB371', '#20B2AA', '#008B8B',
          '#5F9EA0', '#696969', '#708090', '#2F4F4F', '#556B2F'
        ];
        
        // Generate colors for common nicknames
        for (let i = 0; i < 100; i++) {
          const colorIndex = i % darkColors.length;
          const variation = Math.random() * 0.3 - 0.15; // -15% to +15% variation
          const baseColor = darkColors[colorIndex];
          this.nicknameColors.set(i, this.adjustColorBrightness(baseColor, variation));
        }
      }
      ["adjustColorBrightness"](hexColor, factor) {
        const rgb = parseInt(hexColor.slice(1), 16);
        const r = Math.max(0, Math.min(255, Math.floor((rgb >> 16) * (1 + factor))));
        const g = Math.max(0, Math.min(255, Math.floor(((rgb >> 8) & 0x00FF) * (1 + factor))));
        const b = Math.max(0, Math.min(255, Math.floor((rgb & 0x0000FF) * (1 + factor))));
        return '#' + ((r << 16) | (g << 8) | b).toString(16).padStart(6, '0');
      }
      ["getNicknameColor"](nickname) {
        if (!nickname) return '#FFFFFF';
        
        // Check if we already have a color for this nickname
        if (this.nicknameColors.has(nickname)) {
          return this.nicknameColors.get(nickname);
        }
        ...
        const colorIndex = Math.abs(hash) % this.nicknameColors.size;
        const color = Array.from(this.nicknameColors.values())[colorIndex] || '#FFFFFF';
      ...
      ['initClient'](_0x2390ba, _0x5ba875) {
        ...
        window.zynxNotification.info("Establishing connection (" + _0x2390ba + ")", "Connecting");
        const _0x4d98a0 = new _0x2275aa(_0x2390ba);
        _0x4d98a0.connect(_0x5ba875);
        _0x4d98a0.once("clientReady", _0x5e553e => {
          window.zynxNotification.success('Connection established!', "Client (" + _0x2390ba + ')');
        ...
        _0x4d98a0.on("close", _0x3d4a54 => {
          window.zynxNotification.warning("Connection closed.", "Client (" + _0x2390ba + ')');
        ...
        _0x4d98a0.on("playerDied", () => {
          ...
          if (window.app && window.app.settings && window.app.settings.autoShiftMultibox) {
      ...
      ["switchClient"]() {
        // Prevent multiple rapid calls that cause duplicate child connections
        if (this.switchingClient) {
          return;
        }
        this.switchingClient = true;
        setTimeout(() => {
          this.switchingClient = false;
        }, 50);
        ...
        if (_0x5bd2b3 && "parent" === _0x5bd2b3.clientType) {
          if (_0x5bd2b3.playing) {
            if (_0x28d924) {
              ...
              // Check if we already have a child client connecting to prevent duplicates
              if (this.connectingChild) {
                return;
              }
              this.connectingChild = true;
              const _0x2d0f52 = this.initClient('child', this.serverUrl);
              _0x2d0f52.once("clientReady", () => {
                this.connectingChild = false;
                _0x2d0f52.sendSpawn();
      ...
      ['start']() {
        this.generateNicknameColors(); // Generate new colors on each page load
        this.handelResizing();
        this.handleESCKey();
      ...
      ["initPlayerControls"]() {
        ...
        document.addEventListener("keydown", _0x18521b => {
          // Prevent Tab key default behavior and handle it specially
          if ("Tab" === _0x18521b.code) {
            _0x18521b.preventDefault();
            // Only trigger switch action once per keydown
      ...
      ["triggerAction"](_0x47d3fc, _0x1a0bbe) {
        ...
        switch (_0x47d3fc) {
          ...
          case "lineSplitKey":
            ...
            // Calculate splits based on total cell mass for equal-sized cells
            let totalMass = 0;
            if (_0x425a26.stores && _0x425a26.stores.ownedCells) {
              _0x425a26.stores.ownedCells.forEach(cell => {
                totalMass += cell.mass || 0;
              });
            }
            const minMassPerCell = 100; // Minimum mass each cell should have
            const maxSplits = Math.min(Math.floor(totalMass / minMassPerCell) - 1, 16); // Calculate splits based on mass
            let splitCount = 0;
            const splitInterval = setInterval(() => {
              if (splitCount >= maxSplits || !_0x425a26) {
                clearInterval(splitInterval);
                return;
              }
              _0x425a26.sendSplit(0x1);
              splitCount++;
            }, 50); // 50ms between each split for fast line effect
            break;
      ...
      ['triggerMouseAction'](_0x162f0b) {
        ...
        } else if ("lineSplit" === _0x4154b3) {
          if (_0x49b22f) {
            // Calculate splits based on total cell mass for equal-sized cells
            let totalMass = 0;
            if (_0x49b22f.stores && _0x49b22f.stores.ownedCells) {
              _0x49b22f.stores.ownedCells.forEach(cell => {
                totalMass += cell.mass || 0;
              });
            }
            const minMassPerCell = 100; // Minimum mass each cell should have
            ...
            let splitCount = 0;
            const splitInterval = setInterval(() => {
              if (splitCount >= maxSplits || !_0x49b22f) {
                clearInterval(splitInterval);
                return;
              }
              _0x49b22f.sendSplit(0x1);
      ...
      ["initSettingsTabs"]() {
        const _0x65aad7 = document.querySelectorAll('.sidebar-tab');
        ...
        function _0x2b36db(_0x1abf3a) {
          ...
          _0x3688bb.forEach(_0x1823eb => _0x1823eb.classList.remove("active"));
          _0x453345.classList.add("active");
          const _0x4b2d8e = _0x453345.getAttribute("data-tab");
          const _0x5e553e = document.querySelector(`.tab-content[data-tab-content="${_0x4b2d8e}"], .tab-content#${_0x4b2d8e}`);
          if (_0x5e553e) {
            _0x5e553e.classList.add("active");
          }
        }
        _0x65aad7.forEach(_0x1823eb => {
          _0x1823eb.addEventListener("click", _0x2b36db);
        });
      }
      ['initSettings']() {
        this.loadSettings();
        ...
        this.bindToggleSwitch('colorNames', "colorNames");
        ...
        this.bindToggleSwitch("arrowRings", "arrowRings");
        this.bindToggleSwitch('autoShiftMultibox', 'autoShiftMultibox');
        this.bindToggleSwitch('useGpuRenderer', 'useGpuRenderer');
        const _gpuEl = this.getEl('useGpuRenderer');
        if (_gpuEl) {
          _gpuEl.addEventListener('change', () => {
    ...
    document.addEventListener("DOMContentLoaded", () => {
      console.log("Document is ready, initializing ProtonX Engine...");
      _0x2395ab.start();
      // expose app before starting renderer so it can read settings
...
if (arrowColorInput) {
    arrowColorInput.addEventListener('input', () => {
        ...
        try { localStorage.setItem('arrowColor', arrowColor); } catch {}
    });
}
try {
    const saved = localStorage.getItem('arrowColor');
    if (saved) arrowColor = saved;
} catch {}
...
if (arrowSizeInput) {
    arrowSizeInput.addEventListener('input', () => {
        ...
        if (arrowSizeValue) arrowSizeValue.textContent = arrowSize + 'px';
        try { localStorage.setItem('arrowSize', arrowSize); } catch {}
    });
}
try {
    const saved = localStorage.getItem('arrowSize');
    if (saved) {
        arrowSize = parseInt(saved);
        if (arrowSizeInput) arrowSizeInput.value = arrowSize;
        if (arrowSizeValue) arrowSizeValue.textContent = arrowSize + 'px';
    }
} catch {}
...
// 3D Effect handling
const enable3DEffectCheckbox = document.getElementById('enable3DEffect');

// Set up 3D effect toggle
if (enable3DEffectCheckbox) {
    // Load saved state
    try {
        const savedState = localStorage.getItem('enable3DEffect');
        if (savedState !== null) {
            enable3DEffectCheckbox.checked = savedState === 'true';
        }
    } catch {}
    
    // Listen for changes
    enable3DEffectCheckbox.addEventListener('change', () => {
        const isEnabled = enable3DEffectCheckbox.checked;
        try {
            localStorage.setItem('enable3DEffect', isEnabled);
        } catch {}
        
        // Update settings if settings manager exists
        if (window._0x2395ab && window._0x2395ab.settings) {
            window._0x2395ab.settings.enable3DEffect = isEnabled;
        }
    });
    
    // Initialize setting
    if (window._0x2395ab && window._0x2395ab.settings) {
        window._0x2395ab.settings.enable3DEffect = enable3DEffectCheckbox.checked;
    }
}
...
function initializeDefaultBackground() {
    console.log('initializeDefaultBackground called, renderer:', !!window.renderer);
    if (window.renderer && !window.renderer.backgroundImage) {
        const defaultImg = new Image();
        defaultImg.crossOrigin = "anonymous";
        defaultImg.onload = () => {
            console.log('Default Imgur background loaded successfully');
            window.renderer.backgroundImage = defaultImg;
            window.renderer.backgroundImageLoaded = true;
        };
        defaultImg.onerror = () => {
            console.warn('Failed to load default Imgur background');
            window.renderer.backgroundImageLoaded = false;
        };
        defaultImg.src = 'https://i.imgur.com/aKvo1jQ.png';
        console.log('Loading default Imgur background from:', defaultImg.src);
    }
}

// Validate if URL is from Imgur
function isValidImgurUrl(url) {
    if (!url || typeof url !== 'string') return false;
    return url.includes('imgur.com') || url.includes('i.imgur.com');
}

// Load background image with validation
function loadBackgroundImage(url) {
    if (!window.renderer) return;
    
    if (!url || !isValidImgurUrl(url)) {
        console.warn('Invalid or non-Imgur URL provided:', url);
        if (window.zynxNotification) {
            window.zynxNotification.error('Please enter a valid Imgur image URL', 'Background Error');
        }
        return;
    }
    ...
    bgImg.onload = () => {
        console.log('Imgur background loaded successfully:', url);
        window.renderer.backgroundImage = bgImg;
        window.renderer.backgroundImageLoaded = true;
        if (window.zynxNotification) {
            window.zynxNotification.success('Background image loaded!', 'Success');
        }
    };
    bgImg.onerror = () => {
        console.warn('Failed to load Imgur background image:', url);
        window.renderer.backgroundImageLoaded = false;
        if (window.zynxNotification) {
            window.zynxNotification.error('Failed to load Imgur image. Check the URL.', 'Background Error');
        }
    };
    bgImg.src = url;
    console.log('Loading Imgur background from:', url);
}
// Load saved background on start
try {
    const savedBg = localStorage.getItem('customBackground');
    const defaultBg = backgroundInput.value.trim(); // Get the default value from the input
    
    const bgUrl = savedBg || defaultBg; // Use saved or default
    
    if (bgUrl) {
        backgroundInput.value = bgUrl;
        // Load the background image using the new validation function
...
if (backgroundInput) {
    backgroundInput.addEventListener('change', () => {
        ...
        if (url) {
            // Load the background image using the validation function
...
// Initialize background when renderer is ready
function waitForRenderer() {
    if (window.renderer) {
        console.log('Renderer found, setting up Imgur background');
        // Enable background setting by default and synchronize with internal settings
        if (window.app && window.app.settings) {
          window.app.settings.showBackground = true;
          console.log('Background setting enabled:', window.app.settings.showBackground);
        }
        // Some renderer code references an internal settings object (_0x2395ab). Ensure it's synchronized.
        if (window._0x2395ab && window._0x2395ab.settings) {
          // Only set if not explicitly defined to avoid overwriting user prefs
          if (typeof window._0x2395ab.settings.showBackground === 'undefined') {
            window._0x2395ab.settings.showBackground = true;
            console.log('Synchronized internal showBackground setting to true');
          }
        }
        
        // Load the default Imgur background from the input field
        const defaultBg = backgroundInput.value.trim();
        if (defaultBg) {
            console.log('Loading default Imgur background from input:', defaultBg);
            loadBackgroundImage(defaultBg);
        } else {
            initializeDefaultBackground();
        }
    } else {
        console.log('Renderer not ready yet, waiting...');
        setTimeout(waitForRenderer, 100);
    }
}
waitForRenderer();

// Also try to load background after a delay as a backup
setTimeout(() => {
    if (window.renderer && !window.renderer.backgroundImageLoaded && backgroundInput) {
        console.log('Delayed Imgur background load attempt');
        const defaultBg = backgroundInput.value.trim();
        if (defaultBg) {
            loadBackgroundImage(defaultBg);
        }
...
const themeDefaults = {
    ...
    borderRadius: '0',
    uiScale: '100',
    inputFieldColor: '#1a1a1a',
    inputTextColor: '#ffffff'
};

// Theme elements to customize
const themeElements = {
    buttonHover: ['.bar-button:hover', '.tab:hover'],
    textColor: ['#menu-display', '.player-inputs input', '.server-select-input', '.option span'],
    accentColor: ['.setting-slider::-webkit-slider-thumb', '.slider:checked + .slider', '.bar-button-bg.play', '.bar-button-bg.spectate'],
    menuWidth: ['#menu-display-center'],
    menuHeight: ['#menu-display'],
    buttonSize: ['.bar-button .text', '.tab', '.player-inputs input', '.server-select-input'],
    borderRadius: ['#menu-display', '.bar-button', '.tab', '.option', '.player-inputs input', '.server-select-input'],
    uiScale: ['#menu-display'],
    inputFieldColor: ['.player-inputs input', '.server-select-input'],
    inputTextColor: ['.player-inputs input', '.server-select-input']
};
...
function applyTheme() {
    Object.keys(themeDefaults).forEach(key => {
        ...
        if (input) {
            ...
            const elements = themeElements[key];
    ...
    Object.keys(themeDefaults).forEach(key => {
        ...
        if (input) {
            ...
            try {
                ...
                console.warn('Could not clear localStorage for', key, e);
            }
            
            // Update the displayed value for sliders
            const valueDisplay = document.getElementById(key + 'Value');
            if (valueDisplay) {
                let displayValue = themeDefaults[key];
                // Add 'px' suffix for dimension values if not already present
                if ((key.includes('Width') || key.includes('Height') || key.includes('Radius') || key.includes('Size')) && !displayValue.includes('px')) {
                    displayValue += 'px';
                }
...
// New force reset function that specifically sets the correct values
function forceResetTheme() {
    // Force set the specific values you want
    ...
    // Set each specific value
    Object.keys(specificDefaults).forEach(key => {
        const input = document.getElementById(key);
        if (input) {
            input.value = specificDefaults[key];
            ...
            const valueDisplay = document.getElementById(key + 'Value');
            if (valueDisplay) {
                valueDisplay.textContent = specificDefaults[key] + 'px';
            }
        }
    });
    ...
    console.log('Force reset applied: Menu Width=1200px, Menu Height=492px, Border Radius=0px');
}
...
function initTheme() {
    ...
    // New force reset button
    const forceResetBtn = document.getElementById('resetThemeBtnNew');
    if (forceResetBtn) {
        forceResetBtn.addEventListener('click', forceResetTheme);
    }
}
